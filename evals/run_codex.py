#!/usr/bin/env python3
"""Run skill eval prompts against Codex in a clean context (one prompt per process)."""
...
def read_last_message(path: Path) -> str | None:
    if not path.exists():
        return None
    try:
...
def run_prompt(cmd_template: str, model: str, prompt: str) -> tuple[int, str]:
    with tempfile.NamedTemporaryFile(delete=False) as tmp_file:
        last_message_path = Path(tmp_file.name)
    try:
        cmd = cmd_template.format(
            model=shlex.quote(model),
            prompt=shlex.quote(prompt),
        )
        cmd = f"{cmd} --output-last-message {shlex.quote(str(last_message_path))}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run skill eval prompts against Codex.")
    parser.add_argument("--model", required=True, help="Model identifier, used in output path and command template.")
    parser.add_argument("--agent", default="codex", help="Agent name used in output path.")
    ...
    print(f"\n{'='*60}")
    print(f"  CODEX EVAL: {total_skills} skills, {total_prompts} prompts")
